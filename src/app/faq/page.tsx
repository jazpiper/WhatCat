    ...
    items: [
      {
        ...
        answer: 'WhatCat의 테스트는 14개 설문조사 기반 알고리즘을 활용하여 약 80-85%의 정확도를 보입니다. 사용자의 라이프스타일, 성향, 환경을 종합적으로 분석하여 가장 적합한 품종을 추천합니다. 하지만 결과는 참고용으로 활용해주세요.'
      },
      {
        question: '같은 답변으로 다시 테스트하면 결과가 같나요?',
        answer: '네, 같은 답변으로 테스트하면 항상 동일한 결과가 나옵니다. 알고리즘은 결정론적으로 동작합니다.'
      },
      {
        question: '친구와 테스트 결과를 비교할 수 있나요?',
        answer: '네! WhatCat에서 친구와 결과를 공유하고 서로 비교해볼 수 있습니다. 결과 페이지에서 "링크 복사"나 "공유하기"를 통해 쉽게 공유하고 비교 페이지에서 두 결과를 나란히 확인할 수 있습니다.'
      },
      {
        question: '테스트는 어떤 방식으로 진행되나요?',
        answer: '14개의 설문조사 질문에 답변하는 방식으로 진행됩니다. 질문은 라이프스타일, 성격, 관리 용이성, 외형 선호, 비용 등 5대 카테고리로 구성되어 있습니다.'
      },
      {
        question: '테스트에 걸리는 시간은?',
        answer: '14개 질문에 답변하는 데 약 2-3분 정도 걸립니다.'
      },
      {
        question: '테스트 중간에 멈췄다가 다시 할 수 있나요?',
        answer: '현재는 테스트를 중단하면 처음부터 다시 시작해야 합니다. 약 2-3분 소요되므로 한 번에 완료하는 것을 추천합니다.'
      },
      {
        question: '테스트 결과가 마음에 안 들어요.',
        answer: '테스트 결과는 사용자의 답변에 기반한 추천이므로, 다른 품종이 궁금하시다면 답변을 바꾸어 다시 테스트해보시기 바랍니다.'
      },
      {
      ...
      },
      {
        ...
        answer: '서비스는 사용자의 개인정보를 수집하지 않으므로 테스트 결과가 서버에 저장되지 않습니다. 결과를 저장하고 싶으시면 "이미지 저장" 기능을 이용하거나 공유 링크를 저장해주세요.'
      },
      {
        question: '어떤 카테고리로 분석하나요?',
        answer: '5대 카테고리로 분석합니다: 라이프스타일(30%), 성격(25%), 관리 용이성(20%), 외형(15%), 비용(10%)입니다.'
      },
      {
        question: '테스트 결과를 PDF로 저장할 수 있나요?',
        answer: '현재는 결과를 이미지 형태로 저장할 수 있습니다. PDF 다운로드 기능은 추후 추가될 예정입니다.'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: 'WhatCat 서비스는 사용자의 개인정보를 수집하지 않습니다. 모든 테스트 답변은 사용자의 브라우저에만 저장되며, 서버로 전송되지 않습니다. 자세한 내용은 개인정보처리방침을 확인해주세요.'
      },
      {
        question: '서비스 이용 요금은 있나요?',
        answer: 'WhatCat 서비스는 완전 무료로 제공됩니다. 모든 기능을 제한 없이 이용할 수 있습니다.'
      }
    ]
  }
];
