    ...
    <PageContainer contentClassName="max-w-2xl">
        ...
        {/* Share button */}
          {state.streak > 0 && (
            <button
              ...
              className="mt-4 w-full bg-[var(--bg-surface)]/70 text-[var(--text-primary)] hover:bg-[var(--bg-surface)]/90 py-2 rounded-lg transition-colors flex items-center justify-center gap-2"
            >
            <Share2 size={18} />
      ...
      {isCompletedToday && showResult && isCorrect ? (
        <Card className="text-center">
          ...
          <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">오늘의 퀴즈 완료!</h2>
          <p className="text-[var(--text-secondary)] mb-6">내일 다시 도전해주세요</p>

          {/* Countdown */}
          <div className="bg-[var(--bg-page)] rounded-xl p-4 mb-6">
        ...
        <Card>
          ...
          {/* Question */}
          <h2 className="text-xl font-bold text-[var(--text-primary)] mb-6">{quiz.question}</h2>
          ...
          <div className="space-y-3">
            {quiz.options.map((option, index) => {
              ...
              if (!showResult) {
                buttonClass += 'border-[var(--border-default)] hover:border-pink-300 hover:bg-pink-100 cursor-pointer';
              } else if (index === quiz.correctIndex) {
                buttonClass += 'border-green-500 bg-green-100';
              } else if (index === selectedAnswer && !isCorrect) {
                buttonClass += 'border-red-500 bg-red-100';
              } else {
                buttonClass += 'border-[var(--border-default)] opacity-60';
              }
          ...
          {showResult && (
            <div
              ...
              <p className="text-[var(--text-primary)]">{quiz.explanation}</p>
      ...
      {/* Milestones Info */}
      <Section title="연속 도전 업적" emoji="🏆" className="mt-8 bg-[var(--bg-surface)]/80">
              ...
              <div
                ...
                className={`text-center p-3 rounded-xl ${
                  ...
                  ? 'bg-gradient-to-br from-pink-100 to-purple-100 border-2 border-pink-300'
                    : 'bg-[var(--bg-page)] border-2 border-[var(--border-default)]'
                }`}
              >
                <div className="text-2xl mb-1">{milestone.emoji}</div>
                <div className="text-sm font-bold text-[var(--text-primary)]">{milestone.days}일</div>
                <div className="text-xs text-[var(--text-secondary)]">{milestone.title}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* Footer */}
      <div className="text-center text-[var(--text-secondary)] text-xs mt-8">
