...
import { getMBTICode } from '@/utils/catMBTI';
import {
  createShareUrl,
...
export default function ResultClient() {
  ...
  const handleShareKakao = () => {
    ...
    // 동적 OG 이미지 URL 생성
    const ogImageUrl = getKakaoShareImageUrl(firstResult.breed.id, firstResult.score);
    ...
    if (friendLink.trim() && firstResult) {
      try {
        ...
        if (breed2Id && score2) {
          ...
          // MBTI 코드 계산
          const mbti1 = getMBTICode(firstResult.breed);

          // 친구의 품종 찾아서 MBTI 계산
          const friendBreed = breeds.find(b => b.id === breed2Id);
          const mbti2 = friendBreed ? getMBTICode(friendBreed) : '';

          const mbtiParams = mbti2 ? `&mbti1=${mbti1}&mbti2=${mbti2}` : `&mbti1=${mbti1}`;

          router.push(`/compare?breed1=${firstResult.breed.id}&score1=${firstResult.score}&breed2=${breed2Id}&score2=${score2}${mbtiParams}`);
        } else {
