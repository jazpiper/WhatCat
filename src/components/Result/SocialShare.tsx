...
export default function SocialShare({
    ...
    return (
        <div className="bg-white dark:bg-[var(--bg-surface)] rounded-3xl shadow-xl p-6 mb-6 border border-[var(--border-default)] dark:border-[var(--border-default)]">
            <h3 className="text-xl font-bold text-[var(--text-primary)] dark:text-[var(--text-secondary)] mb-4 text-center flex items-center justify-center gap-2">
            ...
            <div className="bg-pink-50 dark:bg-pink-900/20 rounded-2xl p-4 mb-6 border border-pink-100 dark:border-pink-800/50">
                <p className="text-[var(--text-primary)] dark:text-[var(--text-secondary)] text-sm leading-relaxed">{shareCopy}</p>
            </div>
            ...
            <div className={`grid gap-3 ${hasCelebrityMatch && onShareCelebrityStory ? 'grid-cols-3 md:grid-cols-4 lg:grid-cols-7' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6'}`}>
                <button
                    ...
                    className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-pink-300 focus:ring-offset-2 ${isDownloading
                            ? 'bg-[var(--bg-page)] dark:bg-[var(--bg-surface)] text-[var(--text-secondary)] cursor-not-allowed'
                ...
                <button
                    onClick={onShareX}
                    className="flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-black text-white hover:shadow-lg transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-[var(--border-default)] focus:ring-offset-2"
                ...
                <button
                    ...
                    className="flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-[var(--bg-surface)] text-white hover:shadow-lg transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-[var(--border-default)] focus:ring-offset-2"
                ...
                <button
                    onClick={onCopyLink}
                    className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-[var(--border-default)] focus:ring-offset-2 ${copied
                            ? 'bg-green-500 text-white'
                            : 'bg-[var(--bg-page)] dark:bg-[var(--bg-surface)] text-[var(--text-primary)] dark:text-[var(--text-secondary)] hover:shadow-lg hover:bg-[var(--bg-page)] dark:hover:bg-[var(--bg-surface)]'
                        }`}
                    aria-live="polite"
