    ...
    return (
        <Link
            ...
            className={`flex items-center gap-4 p-4 rounded-xl transition-all duration-300 cursor-pointer hover:scale-[1.02] hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-pink-300 focus:ring-offset-2 ${
                index === 0
                    ...
                    : 'bg-[var(--bg-page)] dark:bg-[var(--bg-surface)] border border-[var(--border-default)] dark:border-[var(--border-default)]'
            }`}
        ...
        >
            ...
            <div className="flex-1">
                <h4 className="font-bold text-[var(--text-primary)] dark:text-[var(--text-secondary)]">{result.breed.name}</h4>
                <p className="text-sm text-[var(--text-secondary)] dark:text-[var(--text-secondary)]">{result.breed.nameEn}</p>
            </div>
...
export default function TopRecommended({ results }: TopRecommendedProps) {
    return (
        <div className="border-t-2 border-pink-100 dark:border-pink-900/50 pt-6">
            <h3 className="text-xl font-bold text-[var(--text-primary)] dark:text-[var(--text-secondary)] mb-4 text-center">
                🏆 추천 품종 TOP 3
            </h3>
